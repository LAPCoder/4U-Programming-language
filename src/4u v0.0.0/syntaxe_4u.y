...
%token                  TOK_DEF_VAR     /* definir variable */
...
%token					TOK_AFFECT_PLUS	TOK_AFFECT_MOINS	TOK_AFFECT_MUL	TOK_AFFECT_DIV	TOK_AFFECT_MOD TOK_AFFECT_PUI	/* += -= *= /= %= ^= */
...
affectation:	def_variable variable_entiere TOK_AFFECT expression_arithmetique TOK_FINSTR{
		...
		|
		/*variable_entiere TOK_AFFECT_PUI expression_arithmetique TOK_FINSTR{
			Variable* var=g_hash_table_lookup(table_variable,(char*)g_node_nth_child($1,0)->data);
			if(var==NULL){
				fprintf(stderr,"\tERREUR : Erreur de semantique a la ligne %d. Variable %s jamais declaree !\n",lineno,(char*)g_node_nth_child($1,0)->data);
				error_semantical=true;
			}else{
				$$=g_node_new((gpointer)AFFECTATION_PUI);
				g_node_append($$,$1);
				g_node_append($$,$3);
			}
		}
		...
		|
		def_variable variable_decimale TOK_AFFECT expression_arithmetique TOK_FINSTR{
			/* $2 est la valeur du premier non terminal. Ici c'est la valeur du non terminal variable. $4 est la valeur du 2nd non terminal. */
			printf("\t\tAffectation sur la variable\n");
			Variable* var=g_hash_table_lookup(table_variable,(char*)g_node_nth_child($2,0)->data);
			if(var==NULL){
		...
		|
		variable_decimale TOK_AFFECT_PUI expression_arithmetique TOK_FINSTR{
			Variable* var=g_hash_table_lookup(table_variable,(char*)g_node_nth_child($1,0)->data);
			if(var==NULL){
				fprintf(stderr,"\tERREUR : Erreur de semantique a la ligne %d. Variable %s jamais declaree !\n",lineno,(char*)g_node_nth_child($1,0)->data);
				error_semantical=true;
		...
		|
		def_variable variable_booleenne TOK_AFFECT expression_booleenne TOK_FINSTR{
			/* $2 est la valeur du premier non terminal. Ici c'est la valeur du non terminal variable. $4 est la valeur du 2nd non terminal. */
			printf("\t\tAffectation sur la variable\n");
			Variable* var=g_hash_table_lookup(table_variable,(char*)g_node_nth_child($2,0)->data);
			if(var==NULL){
		...
		|
		def_variable variable_texte TOK_AFFECT expression_texte TOK_FINSTR{
			/* $2 est la valeur du premier non terminal. Ici c'est la valeur du non terminal variable. $4 est la valeur du 2nd non terminal. */
			printf("\t\tAffectation sur la variable\n");
			Variable* var=g_hash_table_lookup(table_variable,(char*)g_node_nth_child($2,0)->data);
			if(var==NULL){
...
/*puissance:		expression_arithmetique TOK_PUISSANCE expression_arithmetique{
				printf("\t\t\tPuissance\n");
				$$=g_node_new((gpointer)PUISSANCE);
				g_node_append($$,$1);
				g_node_append($$,$3);
			};*/
